...
async def list_approval_rules(request: Request) -> List[Dict[str, Any]]:
    await current_user(request)
    return await db.approval_rules.find({}, {"_id": 0}).sort([("doc_type", 1), ("sort", 1)]).to_list(500)


@router.post("/approval-rules")
async def create_approval_rule(payload: ApprovalRulePayload, request: Request) -> Dict[str, Any]:
    actor = await require_permission(request, "entity", "create")
    doc = {"id": new_id("aprule"), **payload.model_dump(),
           "created_at": now_iso(), "updated_at": now_iso()}
    await db.approval_rules.insert_one(doc)
    await audit(actor["name"], "approval_rule_created", "approval_rules", doc["id"], doc)
    return safe_doc(doc)


@router.patch("/approval-rules/{rule_id}")
async def update_approval_rule(rule_id: str, payload: GenericPatch, request: Request) -> Dict[str, Any]:
    actor = await require_permission(request, "entity", "update")
    data = dict(payload.data); data["updated_at"] = now_iso()
    doc = await db.approval_rules.find_one_and_update(
        {"id": rule_id}, {"$set": data}, projection={"_id": 0}, return_document=ReturnDocument.AFTER)
    if not doc:
        raise HTTPException(status_code=404, detail="Rule tidak ditemukan")
    await audit(actor["name"], "approval_rule_updated", "approval_rules", rule_id, data)
    return doc


@router.delete("/approval-rules/{rule_id}")
async def delete_approval_rule(rule_id: str, request: Request) -> Dict[str, Any]:
    actor = await require_permission(request, "entity", "delete")
    res = await db.approval_rules.delete_one({"id": rule_id})
    if res.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Rule tidak ditemukan")
    await audit(actor["name"], "approval_rule_deleted", "approval_rules", rule_id, {})
    return {"message": "Rule dihapus", "id": rule_id}
