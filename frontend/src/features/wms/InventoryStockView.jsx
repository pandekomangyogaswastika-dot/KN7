/**
 * InventoryStockView — WMS Stock Tab (Roll-as-SSOT, Fase 0.5)
 * - Balances (proyeksi) per product × warehouse × OWNER
 * - Rolls tab: daftar roll fisik (SSOT)
...
import { BarChart2, Plus, X, History, RefreshCw, Search, MapPin, Layers, Building2 } from "lucide-react";
...
export default function InventoryStockView({ warehouses = [], products = [], entities = [], selectedEntity = "all", user }) {
  ...
  const fetchBalances = async () => {
    setLoading(true);
    try {
      const [b, m, rl] = await Promise.all([
  ...
  return (
    <div data-testid="inventory-stock-view" className="flex flex-col gap-3">
      ...
      <div className={`grid gap-3 ${selectedRow && tab === "balances" ? "lg:grid-cols-[1fr_300px]" : ""}`}>
        ...
        {tab === "rolls" && (
          <RollsTable loading={loading} rolls={filteredRolls} />
        )}
