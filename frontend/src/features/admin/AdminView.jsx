...
export default function AdminView({
  ...
  const [product, setProduct] = useState({ sku: "", name: "", category: "Batik", variant: "Regular", color: "", motif: "", grade: "A", supplier: "", base_unit: "meter", price: 0, harga_pokok: 0, gramasi: 0 });
        ...
        <div className="section-card">
          ...
          {tab === "entities" && <div className="grid gap-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <select data-testid="admin-entity-type-input" className="field" value={entity.type} onChange={(e) => setEntity({ ...entity, type: e.target.value })}>
                <option value="PT">PT</option>
                <option value="CV">CV</option>
              </select>
              <select data-testid="admin-entity-tax-input" className="field" value={entity.default_tax_mode} onChange={(e) => setEntity({ ...entity, default_tax_mode: e.target.value })}>
          ...
          {tab === "products" && <div className="grid gap-2">
            {[["sku", "SKU"], ["name", "Nama produk"], ["category", "Kategori"], ["variant", "Varian"], ["color", "Warna"], ["motif", "Motif"], ["grade", "Grade"], ["supplier", "Supplier"], ["base_unit", "Base unit"], ["price", "Harga jual"], ["harga_pokok", "Harga pokok (HPP)"], ["gramasi", "Gramasi (gsm)"]].map(([key, ph]) => <input key={key} data-testid={`admin-product-${key}-input`} className="field" placeholder={ph} value={product[key]} onChange={(e) => setProduct({ ...product, [key]: ["price", "harga_pokok", "gramasi"].includes(key) ? Number(e.target.value) : e.target.value })} />)}
