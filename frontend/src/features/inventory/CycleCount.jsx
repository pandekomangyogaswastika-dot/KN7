...
export default function CycleCount({ token, warehouses, products, userRole }) {
  ...
  const loadSessions = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/cycle-count/sessions`, { headers });
  ...
  const loadSession = async (id) => {
    const res = await fetch(`${API}/api/cycle-count/sessions/${id}`, { headers });
    const data = await res.json();
  ...
  const createSession = async () => {
    ...
    setErr("");
    const res = await fetch(`${API}/api/cycle-count/sessions`, {
      method: "POST", headers, body: JSON.stringify(form)
    });
    if (!res.ok) { const d = await res.json(); setErr(d.detail || "Gagal membuat sesi"); return; }
    const data = await res.json();
  ...
  const addItemToSession = async () => {
    ...
    const res = await fetch(`${API}/api/cycle-count/sessions/${selectedSession.id}/items`, {
      method: "POST", headers, body: JSON.stringify(addItem)
    });
    if (!res.ok) { const d = await res.json(); setErr(d.detail || "Gagal tambah item"); return; }
  ...
  const updateActualQty = async (itemId) => {
    ...
    const res = await fetch(`${API}/api/cycle-count/sessions/${selectedSession.id}/items/${itemId}`, {
      method: "PATCH", headers, body: JSON.stringify({ actual_qty: Number(actualQty[itemId]), notes: "" })
    });
    if (!res.ok) { const d = await res.json(); setErr(d.detail || "Gagal update"); return; }
    await loadSession(selectedSession.id);
    setMsg("Qty aktual tersimpan");
  };

  const submitSession = async () => {
    const res = await fetch(`${API}/api/cycle-count/sessions/${selectedSession.id}/submit`, {
      method: "POST", headers
    });
    if (!res.ok) { const d = await res.json(); setErr(d.detail || "Gagal submit"); return; }
    await loadSession(selectedSession.id);
    await loadSessions();
  ...
  const approveSession = async () => {
    const res = await fetch(`${API}/api/cycle-count/sessions/${selectedSession.id}/approve`, {
      method: "POST", headers,
      body: JSON.stringify({ reason: approveReason || "Disetujui sesuai hasil count" })
    });
    if (!res.ok) { const d = await res.json(); setErr(d.detail || "Gagal approve"); return; }
    await loadSession(selectedSession.id);
    await loadSessions();
  ...
  const rejectSession = async () => {
    ...
    const res = await fetch(`${API}/api/cycle-count/sessions/${selectedSession.id}/reject`, {
      method: "POST", headers, body: JSON.stringify({ reason: rejectReason })
    });
    if (!res.ok) { const d = await res.json(); setErr(d.detail || "Gagal reject"); return; }
    await loadSession(selectedSession.id);
    await loadSessions();
  ...
  return (
    <section data-testid="cycle-count-panel" className="section-card">
      ...
      {err && <div className="mx-4 mb-2 rounded-md bg-red-50 border border-red-200 p-2 text-[12px] text-red-700">{err}</div>}
            ...
            {view === "create" && (
              <div className="rounded-md border border-[#EFF0F2] bg-white p-4">
                ...
                <div className="grid gap-2 max-w-md">
                  <select value={form.warehouse_id} onChange={e => setForm({...form, warehouse_id: e.target.value})} className="field" data-testid="cc-warehouse-select">
                ...
                {selectedSession.status === "open" && (
                  <div className="rounded-md border border-[#EFF0F2] bg-[#FAFBFC] p-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <select value={addItem.product_id} onChange={e => setAddItem({...addItem, product_id: e.target.value})} className="field flex-1 min-w-32" data-testid="cc-add-product-select">
